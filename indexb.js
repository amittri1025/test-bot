...
const cheerio = require('cheerio'); // npm install cheerio (very light)
...
bot.on('text', async ctx => {
  ...
  const txt = 
    ...
    `*Price:* ${data.priceDisplay || 'N/A'}\n` +
    `*Seller:* ${escapeMarkdown(data.seller.name)} (${data.seller.phone || 'hidden'})\n` +
    `*Location:* ${data.location?.name || 'N/A'}\n` +
    `*Ad ID:* \`${adId}\``;

  // edit message with txt + parse_mode: 'Markdown'
} else if (r.data?.seller?.phone) {
  // fallback: still show phone even if status not SUCCESS
} else {
  // error or no phone
}
